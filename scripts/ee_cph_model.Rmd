...
mod1.5 <- coxph(Surv(age, status) ~  larvalTreat + strata(adultTreat), data = eelife)
summary(mod1.5)
cox.zph(mod1.5)
...
mod1.7 <- coxph(Surv(age, status) ~  larvalTreat, data = eelife)
summary(mod1.7)
cox.zph(mod1.7)

aictab(list(mod1.0,mod1.1,mod1.2,mod1.3,mod1.4,mod1.5,mod1.6,mod1.7)) 

# mod1.1 is best (AICcWt=0.99)
...
```{r}
mod1.3 <- coxme(Surv(age, status) ~ larvalTreat + adultTreat + 
                  larvalTreat*adultTreat + strata(sex) +
                  (1 | cluster(cageID)), data = eelife)
summary(mod1.3) 
# Interaction not sig.
car::Anova(mod1.3, type="II")
...
library(MuMIn)
# femAvg <- model.avg(list(mod2.2, mod2.3), 
#                     beta = c("none", "sd", "partial.sd"),
#                     data=eelifeF, options(na.action = "na.fail"))
# summary(femAvg)
# femSel <- model.sel(list(mod2.2, mod2.3), 
#                     beta = c("none", "sd", "partial.sd"),
#                     data=eelifeF, options(na.action = "na.fail"))
# summary(femSel)

get.models(femSel, subset = TRUE)
