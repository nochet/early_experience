...
```{r}
cxph1.0 <- coxph(Surv(age, status) ~  larvalTreat + adultTreat + sex, 
                 data = eelife)
summary(cxph1.0) # some interaction effect
cox.zph(cxph1.0)
tab_model(cxph1.0,file="additiveMod.doc")
...
```{r}
tab_model(cxph1.2.1,cxph1.2.2,cph2.1,cph2.2, file="selectedMods.doc")
...
```{r}
cxph1.2.3 <- coxph(Surv(age, status) ~ larvalTreat + adultTreat + 
                     larvalTreat*adultTreat , data = eelife)

cxph1.2.4 <- coxph(Surv(age, status) ~ larvalTreat + adultTreat + 
                     strata(sex), data = eelife)

cxph1.2.5 <- coxph(Surv(age, status) ~  
                     strata(sex), data = eelife)

aictab(list(cxph1.2.1,cxph1.2.2, cxph1.2.3, cxph1.2.4,cxph1.2.5))

anova(cxph1.2.1,cxph1.2.2)
```
