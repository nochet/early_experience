...
# Simple models: Start complex and work downwards
...
# Test effect of cageID

cageID is significant in two diet/replicate combinations (C_DR2 and DR_DR1)
Sex and cageID are non-PH

```{r}
cph5 <- coxph(Surv(age, status) ~ larvalTreat + 
                adultTreat + sex + cageID, 
              data = eelife)
summary(cph5)
#ggforest(cph2, data = eelife)
cox.zph(cph5)
...
# Drop sex
cph6 <- coxph(Surv(age, status) ~ larvalTreat + 
                adultTreat + cageID, 
              data = eelife)
summary(cph6)
#ggforest(cph2, data = eelife)
