...
Only sex is sig
Non-PH: larval diet, cageID, sex, and the interaction term
cageID unlikely to interact with diet
see Fox and Weisberg (2018)

```{r}
# 4-way interaction
cph1 <- coxph(Surv(age, status) ~ larvalTreat + adultTreat + 
                sex + cageID + larvalTreat:adultTreat:sex:cageID, 
              data = eelife)
summary(cph1)
...
# Drop cageID - likely a RE

Interaction is not significant for one combi (cageIDDR_DR1)
Non-PH in larval diet and sex

```{r}
# 3-way interaction (drop cageID in the interaction term)
cph2 <- coxph(Surv(age, status) ~ larvalTreat + adultTreat + 
...
cph3 <- coxph(Surv(age, status) ~ cpd_var, data = eelife)
summary(cph3)
cox.zph(cph3)
...
Seems to favor cph3 - interacting terms

```{r}
anova(cph1,cph2,cph3)
```
...
4. THERE SEEMS TO BE INTERACTION EFFECTS TO WORRY ABOUT
