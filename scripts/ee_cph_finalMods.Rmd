...
# Obtain survival data for best model

```{r}
mf4_data <- survfit(Surv(age, status) ~ larv_adult + strata(sex), eelife)
summary(mf4_data)

surv4 <- surv_summary(mf4_data, data = eelife)

write.csv(surv4, 
          file = "../processed/survDat_bestModel.csv",
          row.names = FALSE) 
```
