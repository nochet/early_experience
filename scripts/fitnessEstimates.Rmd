...
cc <- surv_eggs %>% filter(Diet=="C_C") %>% 
  rename(count="n.risk",death="n.event",fec="n.eggs") %>% 
  select(-n) 
cc <- rbind(data.frame(Diet="C_C",age=0, count=426, death=0,surv=1,fec=0), cc)

cdr <- surv_eggs %>% filter(Diet=="C_DR") %>% 
  rename(count="n.risk",death="n.event",fec="n.eggs") %>% 
  select(-n) 
cdr <- rbind(data.frame(Diet="C_DR",age=0, count=393, death=0,surv=1,fec=0), cdr)

drc <- surv_eggs %>% filter(Diet=="DR_C") %>% 
  rename(count="n.risk",death="n.event",fec="n.eggs") %>% 
  select(-n) 
drc <- rbind(data.frame(Diet="DR_C",age=0, count=625, death=0,surv=1,fec=0), drc)

drdr <- surv_eggs %>% filter(Diet=="DR_DR") %>% 
  rename(count="n.risk",death="n.event",fec="n.eggs") %>% 
  select(-n) 
drdr <- rbind(data.frame(Diet="DR_DR",age=0, count=510, death=0,surv=1,fec=0), drdr)

cc <- with(cc, life.table(x=age, nDx=death, nKx=count, type="cohort", iwidth=1, width12=c(1,1)))
cdr <- with(cdr, life.table(x=age, nDx=death, nKx=count, type="cohort", iwidth=1, width12=c(1,1)))
drc <- with(drc, life.table(x=age, nDx=death, nKx=count, type="cohort", iwidth=1, width12=c(1,1)))
drdr <- with(drdr, life.table(x=age, nDx=death, nKx=count, type="cohort", iwidth=1, width12=c(1,1)))
...
vv <- select(cc.lt, x,nLx) %>% rena
cc <- left_join(cc,vv, by=x)
cc.mx <- with(cc, fec/nLx)
